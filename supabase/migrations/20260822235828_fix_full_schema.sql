-- Full clean schema for Tumo (Orders + Loyalty)
-- This migration creates all necessary tables in correct order with safe trigger creation
...
CREATE TABLE IF NOT EXISTS product_variant_options (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id UUID REFERENCES product_variant_groups(id) ON DELETE CASCADE NOT NULL,
  name TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_variant_groups_product_id ON product_variant_groups(product_id);
CREATE INDEX IF NOT EXISTS idx_variant_options_group_id ON product_variant_options(group_id);
...
-- Safe trigger creation (drop first to avoid duplicate trigger error)
DROP TRIGGER IF EXISTS update_businesses_updated_at ON businesses;
DROP TRIGGER IF EXISTS update_products_updated_at ON products;
DROP TRIGGER IF EXISTS update_orders_settings_updated_at ON orders_settings;
...
COMMENT ON TABLE businesses IS 'Main business entities for multi-tenant setup';
COMMENT ON TABLE orders_settings IS 'Per-business configuration for the orders module';